...
Wasm2Lang.Wasm.Tree.CustomPasses.MetadataSection.rebuildPassRunResult = function (wasmModule, parsedMeta, binaryen) {
  ...
  var /** @const {number} */ versionField = 'number' === typeof rawVersion ? /** @type {number} */ (rawVersion) : 0;
  if (3 !== versionField) {
    // Refuse rather than silently returning empty metadata: a dropped entry
    // does not fail, it emits valid-looking source that has quietly lost a
    // switch dispatch or a loop plan — far harder to diagnose than a stop.
    throw new Error(
      'Wasm2Lang: unsupported ' +
        MS.SECTION_NAME +
        ' version ' +
        String(versionField) +
        ' (expected 3). Re-run the wasm2lang:codegen normalization step to regenerate this binary.'
    );
  }
...
Wasm2Lang.Wasm.Tree.CustomPasses.MetadataSection.buildLoopPositionToPtrList_ = function (wasmModule, binaryen, rootPtr) {
  ...
  Wasm2Lang.Wasm.Tree.TraversalKernel.forEachExpression(
    ...
    /** @param {!Wasm2Lang.Wasm.Tree.TraversalNodeContext} nodeCtx
      ...
      var /** @const {!BinaryenExpressionInfo} */ info = /** @type {!BinaryenExpressionInfo} */ (nodeCtx.expression);
      if (binaryen.LoopId === info.id) {
        out.push(/** @type {number} */ (nodeCtx.expressionPointer));
      }
      return undefined;
    }
  );
